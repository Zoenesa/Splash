    ...
    Public Class SQLcommon
        ...
        Public Sub openHelp(Optional bookmarks As String = "")
            ...
            Catch ex As Exception
                ...
                mdlSQL.ShowError("Gagal. (open_help). Message : " + exception.Message)
                mdlSQL.INSERTLOG("Gagal. (open_help). Message : " + exception.Message, "")
        ...
        Public Function cekNPWP(_npwp9 As String, Optional nama_form As String = "", Optional nama_event As String = "") As Boolean
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                mdlSQL.ShowError("Gagal. (cek_npwp). Message : " + exception.Message)
                mdlSQL.INSERTLOG("Gagal. (cek_npwp). Message : " + exception.Message, "")
                ProjectData.ClearProjectError()
            End Try
            Return flag
        End Function
        ...
        Public Function isProfileExist() As Boolean
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                mdlSQL.INSERTLOG("Gagal. (cek_profile). Message : " + ex.Message, "")
                flag2 = False
                ProjectData.ClearProjectError()
            End Try
            Return flag2
        ...
        Public Function tambahUname(uname As String, password As String, role As String, UserFName As String, Jabatan As String) As Boolean
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                mdlSQL.INSERTLOG("Gagal. (add_user). Message : " + ex.Message, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
            Return flag
        ...
        Public Function updateUser(password As String, uname As String, role As String) As Boolean
            Dim flag As Boolean
            Try
                flag = New MySqlCommand() With {.Connection = mdlSQL.vConn,
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                mdlSQL.INSERTLOG("Gagal. (update_user). Message : " + ex.Message, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
            Return flag
        ...
        Public Function unameOke(ByRef errMsg As String, uname As String) As Boolean
            ...
            Try
                ...
                Dim DbDataAdapter As New MySqlDataAdapter("SELECT `Password` FROM `user` WHERE `Username` = '" + uname + "'", mdlSQL.vConn)
        ...
        Public Function passwordOke(ByRef errMsg As String, password As String, uname As String) As Boolean
            ...
            Try
                Dim flag2 As Boolean = False
                Dim oleDbDataAdapter As New MySqlDataAdapter("SELECT `Password` FROM `user` WHERE `Username` = '" + uname + "'", mdlSQL.vConn)
                Dim dataTable As New DataTable()
        ...
        Public Function updatePassword(password As String, uname As String) As Boolean
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                mdlSQL.INSERTLOG("Gagal. (update_password). Message : " + ex.Message, "")
                flag = False
                mdlSQL.ShowError(ex.Message)
                ProjectData.ClearProjectError()
            End Try
            Return flag
        End Function

        Public Function sql_Custom_Query(ByRef errMsg As String, ByVal Command As String, ByRef dt As DataTable, ByVal Optional Opsi As String = "") As Boolean
            errMsg = ""
            Dim flag As Boolean
            Try
                Dim adapter As New MySqlDataAdapter(Command & " " & Opsi, mdlSQL.vConn)
                dt = New DataTable()
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                errMsg = "Gagal.(common_SQLQUERY), Kesalahan : " + exception.Message
                mdlSQL.INSERTLOG(errMsg, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
        ...
        Public Function SqlCompare(ByRef errMsg As String, ByVal tableName As String, ByVal CompareValue1 As String, ByVal CompareValue2 As String) As Boolean
            Dim flag1 As Boolean
            Try
                Dim flag2 As Boolean = False
                Dim sqlAdapter As New MySqlDataAdapter("SELECT `" & CompareValue1 & "` FROM `" & tableName & "` WHERE `" & CompareValue1 & "` = '" & CompareValue2 & "'", Konektor.mdlSQL.vConn)
                Dim dt As New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dt)
                If dt.Rows.Count > 0 Then
                    If Operators.ConditionalCompareObjectEqual(dt.Rows(0)(0), DirectCast(CompareValue1, Object), False) Then
                        flag2 = True
                    End If
                Else
                    flag2 = False
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                errMsg = "Gagal.(sql_Compare on SQLCommon), Kesalahan : " + exception.Message
                mdlSQL.INSERTLOG(errMsg, "")
                flag1 = False
                ProjectData.ClearProjectError()
            End Try
        ...
        Public Function getUsers(ByRef errMsg As String, ByRef dtUsers As DataTable, Optional opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT * FROM `user` " + opsi, mdlSQL.vConn)
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                errMsg = "Gagal.(common_get_user), Kesalahan : " + exception.Message
                mdlSQL.INSERTLOG(errMsg, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
        ...
        Public Function LoadTable(ByRef errMsg As String, ByRef dt As DataTable, ByVal tablename As String, Optional ByVal opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT * FROM `" & tablename & "` " + opsi, mdlSQL.vConn)
                dt = New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dt)
                flag = True
            Catch ex As Exception
        ...
        Public Function GetSqlInvoiceData(ByRef errMsg As String, ByRef dt As DataTable, Optional ByVal Opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT * FROM `invoicedata` " & Opsi, mdlSQL.vConn)
                dt = New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dt)
                flag = True
            Catch ex As Exception
        ...
        Public Function LoadTableSchema(ByRef errMsg As String, ByRef dt As DataTable, ByVal schema As String, Optional ByVal opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT TABLE_NAME, TABLE_ROWS FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_SCHEMA = " & schema & opsi, mdlSQL.vConn)
                dt = New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dt)
                flag = True
            Catch ex As Exception
        ...
        Public Function getCustomerSupplier(ByRef errmsg As String, ByRef dt As DataTable, Optional Opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqladapter As New MySqlDataAdapter("Select * FROM `ref_client` " + Opsi, mdlSQL.vConn)
                dt = New DataTable()
                DirectCast(sqladapter, DbDataAdapter).Fill(dt)
                flag = True
            Catch ex As Exception
        ...
        Public Function editSupplier(ByVal ID As Object, ByVal Nama As Object, ByVal Kategori As Object,
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                mdlSQL.INSERTLOG("Gagal. (Update_Supplier). Message : " + ex.Message, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
            Return flag
        ...
        Public Function GetCustomerName(ByRef errMsg As String, ByVal Nama As String) As Boolean
            ...
            Try
                Dim flag2 As Boolean = False
                Dim sqlAdapter As New MySqlDataAdapter("SELECT `Client_Name` FROM `ref_client` WHERE `Client_Name` = '" & Nama & "'", mdlSQL.vConn)
        ...
        Public Function TambahCustomerSupplier(ByVal ID As Object, ByVal npwp As Object,
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                mdlSQL.INSERTLOG("Gagal. (get_Insert_CustomersSupplier). Message : " + exception.Message, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
            Return flag
            ...
            Try
                flag = New MySqlCommand() With {.Connection = mdlSQL.vConn,
                    .CommandText = String.Concat(New String() {"UPDATE `REF_CLIENT` SET `CLIENT_NAME` = '", ADD_QUOTE_ON_SQL(Nama),
                                                 ...
                                                 "', `CLIENT_MAIL` = '", ADD_QUOTE_ON_SQL(Email), "', " & "`CLIENT_USEREDIT` = '" & ADD_QUOTE_ON_SQL(mdlSQL.UserLogin) & "', " &
        ...
        Public Function GetPelangganSupplier(ByRef errMsg As String,
            ...
            errMsg = ""
            Try
                Dim sqladapter As New MySqlDataAdapter(("SELECT  * FROM `ref_Client` " & Opsi & " ORDER BY `ID_NUM`;"), mdlSQL.vConn)
        ...
        Public Function getListWO(ByRef errMsg As String, ByRef dt As DataTable, Optional ByVal opsi As String = "") As Boolean
            ...
            errMsg = ""
            Try
                Dim sqladapter As New MySqlDataAdapter(("SELECT * FROM `listworkorder` " & opsi), mdlSQL.vConn)
                dt = New DataTable
                sqladapter.Fill(dt)
                flag = True
            Catch ex As Exception
        ...
        Public Function GetDataBAST(ByVal errMsg As String, ByRef dt As DataTable, Optional ByVal Opsi As String = "") As Boolean
            Dim flag As Boolean
            Try
                Dim sqladapter As New MySqlDataAdapter(("SELECT * FROM `BAST` " & Opsi), mdlSQL.vConn)
                dt = New DataTable
                sqladapter.Fill(dt)
                flag = True
            Catch ex As Exception
        ...
        Public Function ValidasiTanggal(ByVal tgl As String) As Boolean
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim excep As Exception = ex
                mdlSQL.INSERTLOG(("Gagal. (validasi.Tanggal).Message:" & excep.Message), "")
                flag = False
                ProjectData.ClearProjectError()
                Return flag
                ProjectData.ClearProjectError()
        ...
        Public Function pastingLine(ByRef errMsg As String, ByVal strLine As String, ByRef arrSukses As String(), ByRef errParsing As String) As Boolean
            ...
            Catch ex As Exception
                ...
                mdlSQL.INSERTLOG("Gagal. (parsing_line). Message : " + excep.Message, "")
                flag = False
                ProjectData.ClearProjectError()
                Return flag
                ProjectData.ClearProjectError()
        ...
        Public Function CekNoInvoiceada(ByVal invNo As String, ByVal clientName As String) As Boolean
            Dim flag As Boolean
            Try
                Dim adapter As New MySqlDataAdapter(("SELECT `invoiceno` FROM `invoicedata` WHERE `invoiceno` = '" & stringSQL.ADD_QUOTE_ON_SQL(invNo) & "' AND `invoiceClient` = '" & stringSQL.ADD_QUOTE_ON_SQL(clientName)), mdlSQL.vConn)
                Dim datatable As New DataTable
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim excep As Exception = ex
                mdlSQL.INSERTLOG("Gagal. (cek_existing_Invoice).Message : " + excep.Message, "")
                flag = False
                ProjectData.ClearProjectError()
                Return flag
                ProjectData.ClearProjectError()
        ...
        Public Function GetPropinsi(ByRef errMsg As String, ByRef dtUsers As DataTable, Optional opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT * FROM `provinces` " + opsi, mdlSQL.vConn)
                dtUsers = New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dtUsers)
                flag = True
            Catch ex As Exception
        ...
        Public Function GetKabupaten(ByRef errMsg As String, ByRef dtKabupaten As DataTable, Optional opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT * FROM `regencies` " + opsi, mdlSQL.vConn)
        ...
        Public Function GetKecamatan(ByRef errMsg As String, ByRef dtKabupaten As DataTable, ByVal Command As String, Optional opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter(Command + opsi, mdlSQL.vConn)
                dtKabupaten = New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dtKabupaten)
                flag = True
            Catch ex As Exception
        ...
        Public Function LoadDataBarang(ByRef errMsg As String, ByRef dtTabel As DataTable, ByVal command As String, Optional ByVal Opsi As String = "") As Boolean
            ...
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter(command & " " & Opsi, Konektor.mdlSQL.vConn)
        ...
        Public Function isDataHGBOke(ByRef errMsg As String, ByVal ValueHGB As String) As Boolean
            Dim flag1 As Boolean
            errMsg = ""
            Try
                Dim flag2 As Boolean = False
                Dim sqlAdapter As New MySqlDataAdapter("SELECT `datahgbno` FROM datahgb WHERE datahgbno = '" & ValueHGB & "'", Konektor.mdlSQL.vConn)
                Dim dt As New DataTable()
                DirectCast(sqlAdapter, DbDataAdapter).Fill(dt)
                If dt.Rows.Count = 0 Then
                    flag2 = True
                Else
                    errMsg = "Data HGB Sudah DiInput"
                    flag2 = False
                End If
                flag1 = flag2
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                errMsg = "Gagal. (get_dataHGB on SQLCommon). Kesalahan : " + exception.Message
                mdlSQL.INSERTLOG(errMsg, "")
                flag1 = False
                ProjectData.ClearProjectError()
            End Try
        ...
        Public Function GetDataHGB_dan_Kavling(ByRef errMsg As String, ByRef dt As DataTable, Optional ByVal Opsi As String = "") As Boolean
            errMsg = ""
            Dim flag As Boolean
            Try
                Dim sqlAdapter As New MySqlDataAdapter("SELECT " &
                                              "datahgb.datahgbno AS `NO HGB (DATA HGB)`, " &
                                              "datakavling.kavling_nama AS `NO KAVLING (DATA KAVLING)`, " &
                                              "datahgb.statushgb AS `STATUS HGB`, " &
                                              "datakavling.statuskavling AS `STATUS KAVLING` FROM `datakavling` INNER JOIN `datahgb`" & Opsi, Konektor.mdlSQL.vConn)
                dt = New DataTable()
            ...
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                errMsg = "Gagal.(getDataHGB_Kavling on SQLCommon). Kesalahan : " + exception.Message
                mdlSQL.INSERTLOG(errMsg, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
        ...
        Public Function GetNoHGB_Only(ByRef errMsg As String, ByRef dtHGB As DataTable, Optional ByVal Opsi As String = "") As Boolean
            Dim flag As Boolean
            errMsg = ""
            Try
                Dim sqladapter As New MySqlDataAdapter("SELECT * FROM `datahgb` " & Opsi & "ORDER BY `iddatahgb`", Konektor.mdlSQL.vConn)
                dtHGB = New DataTable()
                DirectCast(sqladapter, DbDataAdapter).Fill(dtHGB)
                flag = True
            Catch ex As Exception
                ProjectData.SetProjectError(ex)
                Dim exception As Exception = ex
                errMsg = "Gagal.(get_HGB on SQLCommon), Kesalahan : " + exception.Message
                mdlSQL.INSERTLOG(errMsg, "")
                flag = False
                ProjectData.ClearProjectError()
            End Try
