...
Public Class common
    ...
    Public Sub openHelp(Optional bookmarks As String = "")
        Dim fileName As String = "User Manual\User Manual eSPT PPN 1111.doc"
        Try
    ...
    Public Function isProfileExist() As Boolean
        ...
        Try
            Dim MysqlDataReader As MySqlDataReader = New MySqlCommand() With { _
                .Connection = mdlCom.vConn, _
                 .CommandText = "SELECT `npwp` FROM `ref_profile`" _
            }.ExecuteReader()
    ...
    Public Function tambahUname(uname As String, password As String, role As String, UserFName As String, Jabatan As String) As Boolean
        Dim flag As Boolean
        Try
            flag = New MySqlCommand() With { _
                .Connection = mdlCom.vConn, _
                .CommandText = ("INSERT INTO `user` (`Username`, `Password`, `kdgroup`, `nip_user`, `UserFname`, `uaktif`, `UserRole`, `JobDesk`) VALUES('" + _
                                uname + "', '" + password + "','" + Strings.Mid(role, 1, 2) + "','" + role + "','" + UserFName + "','" + "T" + "', '" + Strings.Mid(role, 1, 2) + "', '" + Jabatan + "')") _
            }.ExecuteNonQuery() > 0
        Catch ex As Exception
            ProjectData.SetProjectError(ex)
    ...
    Public Function updateUser(password As String, uname As String, role As String) As Boolean
        Dim flag As Boolean
        Try
            flag = New MySqlCommand() With {.Connection = mdlCom.vConn, _
                .CommandText = ("UPDATE `user` SET `Password` = '" + password + "', `kdgroup` = '" + Strings.Mid(role, 1, _
    ...
    Public Function updatePassword(password As String, uname As String) As Boolean
        ...
        Try
            flag = New MySqlCommand() With { _
                .Connection = mdlCom.vConn, _
                .CommandText = ("UPDATE `user` SET `Password` = '" + password + "' WHERE `Username` = '" + uname + "'") _
            }.ExecuteNonQuery() > 0
        Catch ex As Exception
            ProjectData.SetProjectError(ex)
    ...
    Public Function getInfo(ByRef errMsg As String, ByRef retData As String()) As Boolean
        ...
        Try
            ...
            Dim MysqlDataReader As MySqlDataReader = New MySqlCommand() With { _
                .Connection = mdlCom.vConn, _
                .CommandText = "SELECT npwp, nama_wp_profile, alamat, telepon FROM ref_profile" _
            }.ExecuteReader()
            If MysqlDataReader.HasRows Then
    ...
    Public Function getCustomerSupplier(ByRef errmsg As String, ByRef dt As DataTable, Optional Opsi As String = "") As Boolean
        ...
        Return flag
    End Function
    Public Function editSupplier(ByVal ID As Object, ByVal Nama As Object, ByVal Kategori As Object, _
                               ByVal owner As Object, ByVal alamat As Object, ByVal telpon As Object) As Boolean
        Dim flag As Boolean
        Try
            flag = New MySqlCommand() With {.Connection = mdlCom.vConn, _
                    .CommandText = String.Format("UPDATE SUPPLIER SET SUP_NAMASUPPLIER = '" + Nama + "', SUP_KATEGORI = '" + Kategori + _
                                         "', SUP_OWNER = '" + owner + "', SUP_ALAMAT = '" + _
    ...
    Public Function TambahCustomerSupplier(ByVal ID As Object, ByVal npwp As Object,
                                           ByVal Customer As Object, ByVal alamat As Object, ByVal Propinsi As Object, _
                                ByVal Kota As Object, ByVal kecamatan As Object, ByVal telpon As Object, ByVal kodepos As Object) As Boolean
        Dim flag As Boolean
        Try
            flag = New MySqlCommand() With {.Connection = mdlCom.vConn, _
                    .CommandText = String.Format("INSERT INTO `ref_client` VALUES ('{0}', '{1}', '{2}', '{3}', '{4}', '{5}')", _
            RuntimeHelpers.GetObjectValue(ID), RuntimeHelpers.GetObjectValue(npwp), _
            RuntimeHelpers.GetObjectValue(Customer), RuntimeHelpers.GetObjectValue(alamat), RuntimeHelpers.GetObjectValue(Propinsi), RuntimeHelpers.GetObjectValue(Kota), _
    ...
    Public Function UpdatePelangganSupplier(ByVal RecordNo As Object, ByVal Nama As Object, ByVal NPWP As Object, _
                             ByVal Alamat As Object, ByVal Kota As Object, ByVal Propinsi As Object, _
                             ByVal Kecamatan As Object, ByVal Kelurahan As Object, ByVal Kodepos As Object, _
                             ByVal Telpon As Object, ByVal Email As Object
                             ) As Boolean
        Dim flag As Boolean
        Try
            flag = New MySqlCommand() With {.Connection = mdlCom.vConn, _
                    .CommandText = String.Concat(New String() {"UPDATE `REF_CLIENT` SET `CLIENT_NAME` = '", ADD_QUOTE_ON_SQL(Nama), _
                                                 "', `CLIENT_IDTAX` = '", ADD_QUOTE_ON_SQL(NPWP), "', `CLIENT_ADDRESS` = '", ADD_QUOTE_ON_SQL(Alamat), _
                                                 "', `CLIENT_CITY` = '", ADD_QUOTE_ON_SQL(Kota), "', `CLIENT_STATE` = '", ADD_QUOTE_ON_SQL(Propinsi), _
                                                 "', `CLIENT_DISTRICT` = '", ADD_QUOTE_ON_SQL(Kecamatan), "', `CLIENT_VILLAGE` = '", ADD_QUOTE_ON_SQL(Kelurahan), _
                                                 "', `CLIENT_ZIPCODE` = '", ADD_QUOTE_ON_SQL(Kodepos), "', `CLIENT_PHONE` = '", ADD_QUOTE_ON_SQL(Telpon), _
